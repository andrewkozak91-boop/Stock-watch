...
UNIVERSE = [
    # seed with liquid, sub-$30 names to start; grow this list anytime
    ...
    "UPST","ENVX","MVIS","NU","U","QS","OPEN","COUR","ARMK","ABNB"  # keep/trim as you like
]
PRICE_MAX = 30.0
...
def get_profile(symbol):
    # profile2 has ADR flag & shareOutstanding (proxy for float when float missing)
...
def get_15m_volume(symbol):
    # last single 15-min bar
    now = int(time.time())
    frm = now - 60*60*6  # last 6 hours to be safe (market hours)
...
def has_real_catalyst(symbol):
    # lightweight check over recent news headlines
    try:
        since = int((datetime.utcnow()-timedelta(days=7)).timestamp())
        news = fh(f"{BASE}/company-news", {"symbol": symbol, "from": datetime.utcfromtimestamp(since).strftime("%Y-%m-%d"), "to": datetime.utcnow().strftime("%Y-%m-%d")})
...
def classify_tier(is_adr, catalyst_kind):
    # ADRs only Tier-2+; spec PRs are Tier-3 only
...
def derive_trigger(symbol, price):
    # simple placeholder: trigger 2% over current price; you can replace with real level detection
...
def scan_symbol(symbol):
    ...
    if "shareOutstanding" in profile and profile["shareOutstanding"]:
        # rough proxy; you can swap to a true float endpoint if your plan supports it
        free_float = float(profile["shareOutstanding"]) * 1_000_000  # convert M to shares (approx)
    if free_float and free_float > FLOAT_MAX:
        # allow override if institutional-grade catalyst present (handled below)
        pass
    ...
    # override float rule only if institutional-grade catalyst present
    if free_float and free_float > FLOAT_MAX and catalyst_kind != "Real":
        return None

    # ADRs allowed only Tier-2+
    # Speculative PRs => Tier-3 tiny size (we mark but still show)
    if not vol_ok:
        # we keep for watch if close to trigger and improving volume; else drop
        pass

    # VWAP status proxy: price vs prev close (true intraday VWAP needs tick data)
    ...
    # rank score (simple): closer to trigger + has real catalyst + vol gate
    base_score = 0
    try:
